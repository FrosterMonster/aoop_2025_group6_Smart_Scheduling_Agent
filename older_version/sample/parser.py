...
// parseTag extracts the tool calling tag from the template
...
func parseTag(tmpl *template.Template) string {
    ...
    tagContent := content[start+len("{{tool_calls") : start+end]
    tagContent = strings.TrimSpace(tagContent)

    // if no tag specified, return empty string
    if tagContent == "" {
        return ""
    }

    // return the first character of the tag content
    return string(tagContent[0])
}
...
const (
    thinkingState_LookingForTag thinkingState = iota
...
func (s thinkingState) String() string {
    switch s {
    case thinkingState_LookingForTag:
        return "LookingForTag"
...
func (s *Parser) AddContent(content string) (string, string) {
    ...
    for {
        switch s.state {
        case thinkingState_LookingForTag:
            idx := strings.Index(s.acc.String(), s.OpeningTag)
            if idx == -1 {
                remainingSb.WriteString(s.acc.String())
                s.acc.Reset()
                return thinkingSb.String(), remainingSb.String()
            }
            remainingSb.WriteString(s.acc.String()[:idx])
            s.acc.Reset()
            s.acc.WriteString(s.acc.String()[idx+len(s.OpeningTag):])
            s.state = thinkingState_ThinkingStartedEatingWhitespace

        case thinkingState_ThinkingStartedEatingWhitespace:
            trimmed := strings.TrimLeftFunc(s.acc.String(), isWhitespace)
            skipped := len(s.acc.String()) - len(trimmed)
            s.acc.Reset()
        ...
        case thinkingState_Thinking:
            idx := strings.Index(s.acc.String(), s.ClosingTag)
            if idx == -1 {
                thinkingSb.WriteString(s.acc.String())
                s.acc.Reset()
                return thinkingSb.String(), remainingSb.String()
            }
            thinkingSb.WriteString(s.acc.String()[:idx])
            s.acc.Reset()
            s.acc.WriteString(s.acc.String()[idx+len(s.ClosingTag):])
...
func (s *Parser) extractContent() (thinking string, untrimmed string, done bool) {
    switch s.state {
    case thinkingState_LookingForTag:
        idx := strings.Index(s.acc.String(), s.OpeningTag)
        if idx == -1 {
